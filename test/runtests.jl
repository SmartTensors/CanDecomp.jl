...
T = CanDecomp.toT(A, B, C)

for optmethod in [:nnoptim, :nnmads, :nnjump]
	@Base.Test.test CanDecomp.optim_f(C[1, :], 1, T[:, :, 1], StaticArrays.SVector(A, B, C), CanDecomp.Tdims(A, B, C), 0e0) ≈ 0 atol=1e-9
	Cest = similar(C)
	for i_3 = 1:size(C, 1)
		Cest[i_3, :] = CanDecomp.estimatecolumnoflastmatrix(i_3, T[:, :, i_3], StaticArrays.SVector(A, B, zeros(size(C)...)), CanDecomp.Tdims(A, B, C), Val{optmethod}; regularization=1e-3, print_level=0)
	end
	@show Cest, C
	@Base.Test.test T ≈ CanDecomp.toT(A, B, Cest) atol=1e-2
	@Base.Test.test Cest ≈ C atol=2e-2

	Cest2 = zeros(size(C)...)
	matrices = StaticArrays.SVector(A, B, Cest2)
	dims = CanDecomp.Tdims(A, B, C)
	...
	T_oneiteration = CanDecomp.toT(Ap1, Bp1, Cp1)
	...
	T_done = CanDecomp.toT(Apf, Bpf, Cpf)
